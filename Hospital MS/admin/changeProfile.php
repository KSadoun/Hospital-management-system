...
<!--Start: header-->
<center><h3 class="heading text-muted">ADMIN | EDIT DOCTOR PROFILE</h3></center>
<!--end: header-->
<?php 
require 'dbh.php';
  if(isset($_GET['row'])){
    $rowNo = $_GET['row']; 
    $sql = "SELECT * FROM doctors WHERE doctor_no = $rowNo";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    echo '<div class="edit-box bg-light p-5">';
    echo '<form action="changeProfile.inc.php?row='.$row['doctor_no'].'" class="edit-form p-4" method="POST">';
  }
  else{
    echo '<div class="edit-box bg-light p-5">';
    echo '<form action="changeProfile.inc.php?" class="edit-form p-4" method="POST">';
  }
?>
<!--Start: Form content-->
    ...
    <div class="information">
      <!--heading-->
      <center><h6 class="patient-name text-primary" >Doctor's Profile</h6></center><br>
...
require 'dbh.php';
  ...
  if (isset($_GET['specialization'])) {
    ...
    echo '<p class="text-muted">Doctor Specialization</p>
      <select class="specialization" name="specialization">';
      echo '<option>'.$row1['doctor_specialization'].'</option>';
      require 'dbh.php';
      $sql = "SELECT * FROM specializations WHERE display = 'yes'";
      $result = mysqli_query($conn, $sql);
      if (mysqli_num_rows($result)) {
  ...
  if (isset($_GET['doctor'])) {
    ...
    echo '<p class="text-muted">Doctor Name</p>
      <input type="text" class="doctor" name="doctor" value="'.$row1['doctor_fullname'].'"><br><br>';
  }
  ...
  if (isset($_GET['address'])) {
    ...
    echo '<p class="text-muted">Clinic Address</p>
    <input type="text" class="address" name="address" value="'.$row1['doctor_address'].'"><br><br>';
  }
  ...
  if (isset($_GET['fee'])) {
    ...
    echo '<p class="text-muted">Doctor Consultancy Fees</p>
      <input type="text" class="fee" name="fee" value="'.$row1['doctor_fee'].'"><br><br>';
  }
  ...
  if (isset($_GET['number'])) {
    ...
    echo '<p class="text-muted">Doctor Contanct Number</p>
    <input type="number" class="number" name="number" placeholder="Current Number" value="'.$row1['doctor_number'].'"><br><br>';    
  }
  ...
  if (isset($_GET['email'])) {
    ...
    echo '<p class="text-muted">Doctor Email Address</p>
    <input type="email" class="email" name="email" placeholder="Current Email Address" value="'.$row1['doctor_email'].'"><br><br>';
  }
?>
...
?>      
    ...
    <center><p class="text-muted" style="display: inline">Done? </p><a href="doctors.php">Return to the previous site</a></center> 
  </form>
</div>
<!--End: Form content-->
