...
if (isset($_POST['submit'])) {
  ...
  if (empty($specialization) || empty($doctor) || empty($fee) || empty($address) || empty($number) || empty($email)) {
    header("Location: changeProfile.php?err=empty&specialization=$specialization&doctor=$doctor&fee=$fee&address=$address&number=$number&email=$email");
    exit();
  }
  //if name is valid
  elseif (!preg_match("/^[a-zA-Z ]*$/", $doctor)) {
    header("Location: changeProfile.php?err=doctor&specialization=$specialization&fee=$fee&address=$address&number=$number&email=$email");
    exit();
  } 
  //if email is valid
  elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    header("Location: changeProfile.php?err=email&specialization=$specialization&doctor=$doctor&fee=$fee&address=$address&number=$number");
    exit();
  }
  elseif (validate_phone_number($number) == false) {
    header("Location: changeProfile.php?err=number&specialization=$specialization&doctor=$doctor&fee=$fee&address=$address&email=$email");
    exit();
  }
  else{
    ...
    $sql = "UPDATE doctors 
            ...
            WHERE doctor_no = '{$_SESSION['doctor_no']}'";
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      header("Location: changeProfile.php?err=none");
