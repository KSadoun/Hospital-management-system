...
<?php
require 'dbh.php';
$sql = "SELECT * FROM appointments WHERE patient_no = '{$_SESSION['patient_no']}' AND display = 'yes'";
...
if (mysqli_num_rows($result) > 0) {
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo "<td>".$status."</td>";
    echo "<td><button class='btn btn-primary btn-sm' data-number='".$rowNo."' data-row='".$row['appointment_no']."'>Delete</button></td>";
    echo "</tr>";
    $rowNo += 1;
  }
}
...
<script>
  ...
  $('table').on('click', 'button', function(){
    ...
    if (confirm("Are you sure you want to delete Appointment No. " + this.dataset.number)) {
      var row = this.dataset.row;
      $.get( "deleteAppointment.php?row=" + row );
